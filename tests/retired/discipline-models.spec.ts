...
const wrapperClass={software:'object-code',hardware:'object-chip',hybrid:'object-hybrid'} as const;
const model=(page:Page,kind:typeof kinds[number])=>page.locator(`.type-object .discipline-model[data-kind="${kind}"]`);
// The model itself mounts lazily (see Deferred.tsx), so tests must scroll its always-present
// wrapper into view first rather than the not-yet-existent `.discipline-model` locator.
const wrapper=(page:Page,kind:typeof kinds[number])=>page.locator(`.type-object.${wrapperClass[kind]}`);
...
test('mobile discipline models render as their cards enter view without horizontal overflow',async({page})=>{
  ...
  await page.goto('/');
  for(const kind of kinds){
    await wrapper(page,kind).scrollIntoViewIfNeeded();
    const scene=model(page,kind);
