...
test('video embeds only use supported player hosts and safe media protocols',()=>{
  for(const url of ['https://youtu.be/aqz-KE-bpKQ','https://www.youtube.com/watch?v=aqz-KE-bpKQ','https://www.youtube.com/shorts/aqz-KE-bpKQ'])expect(videoSource({videoUrl:url,videoId:''})).toMatchObject({kind:'embed',url:expect.stringMatching(/^https:\/\/www.youtube-nocookie.com\/embed\/aqz-KE-bpKQ\?autoplay=1&mute=1/)});
  for(const url of ['javascript:alert(1)','https://youtube.com.evil.test/watch?v=aqz-KE-bpKQ','file:///movie.mp4','https://example.test/page'])expect(videoSource({videoUrl:url,videoId:''})).toBeNull();
  expect(videoSource({videoUrl:'https://example.test/demo.webm?token=public',videoId:''})).toMatchObject({kind:'file'});
});

test('an uploaded video id always takes priority over a pasted URL',()=>{
  expect(videoSource({videoUrl:'https://youtu.be/aqz-KE-bpKQ',videoId:'a10cd424-dcda-4b15-8a3b-7845e40ae0fe'})).toEqual({kind:'file',url:'/api/files/a10cd424-dcda-4b15-8a3b-7845e40ae0fe'});
  expect(videoSource({videoUrl:'',videoId:'a10cd424-dcda-4b15-8a3b-7845e40ae0fe'})).toEqual({kind:'file',url:'/api/files/a10cd424-dcda-4b15-8a3b-7845e40ae0fe'});
});
