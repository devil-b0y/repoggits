...
import { ArrowUpRight, Plus, Bell, Check, Search, MoreHorizontal } from 'lucide-react';
...
// Rename writes through the same endpoint the editor uses, which accepts a draft or a version sent back for changes.
const canRename=(p:Project)=>['draft','changes_requested'].includes(p.version.status)&&!p.archived;
...
function CardMenu({project,onRename,onShare,shared}:{project:Project;onRename:()=>void;onShare:()=>void;shared:boolean}) {
 ...
 const rename=canRename(project),share=canShare(project);
