  ...
  return <div className="submission-studio">
    <header className="studio-hero">
      <div className="studio-hero-copy">
        ...
        <span className="studio-eyebrow"><NotebookPen size={14}/> THE PROJECT STUDIO <span>{editing?'NEXT VERSION':'NEW PROJECT'}</span></span>
        <h1>Your idea.<br/><em>Its next chapter.</em></h1>
        <p>You made something worth sharing. Give it a home—with the demo, the people, and the story behind the build.</p>
        <a className="studio-start" href="#studio-section-1">Let’s tell your story <ArrowUpRight size={18}/></a>
        <div className="studio-hero-note"><NotebookPen size={15}/><span>Start anywhere. Save a draft. Make it yours.</span></div>
      </div>
      <div className="studio-hero-art" aria-hidden="true">
        <div className="studio-orbit"/>
        <div className="studio-paper paper-back"><Code2 size={27}/><i/><i/><i/></div>
        <div className="studio-paper paper-front">
          <span><span className="studio-preview-dot"/> A LITTLE PREVIEW OF WHAT’S NEXT</span>
          <div className={`studio-art-mark ${data.coverId?'has-image':''}`}>{data.coverId?<img src={`/api/files/${data.coverId}`} alt=""/>:<><Layers size={58}/><Sparkles size={24}/><span>IDEA → BUILD → SHARE</span></>}</div>
          <strong>{data.title.trim()||'Something worth sharing.'}</strong>
          <small>{data.type} <span>BY {data.teamName.trim()||'YOUR TEAM'}</span></small>
        </div>
        <span className="studio-art-note"><Check size={14}/> Made by you. Built to inspire.</span>
      </div>
