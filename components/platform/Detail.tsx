 ...
 return <div className="page-wrap detail-page">
  ...
  <div className="detail-layout"><div>
    ...
    <div className="build-facts">
      ...
      <div><Code2 size={19}/><span>Languages</span>{d.stack.languages?<strong className="language-tags">{d.stack.languages.split(',').map(name=>name.trim()).filter(Boolean).map((name,i,arr)=>{const logo=findLanguageLogo(name);return <span className="language-tag" key={`${name}-${i}`}>{logo&&<span className="language-tag-icon"><img src={logo} alt="" width={13} height={13} loading="lazy"/></span>}<span>{name}{i<arr.length-1?', ':''}</span></span>;})}</strong>:<strong>Not specified</strong>}</div>
    </div>
    {d.features.length>0&&<section className="key-features"><h3>Key Features</h3><ul className="key-features-list">{d.features.map((feature,i)=><li key={i}><CheckCircle2 size={15}/><span>{feature}</span></li>)}</ul></section>}
    <section className="detail-section"><h2>About the project</h2><p className="preserve-lines">{d.description||'The project story is still being written.'}</p></section>
    <section className="detail-section"><h2>Project Team</h2><p>The team behind {d.title.split(' — ')[0]}.</p><div className="team-grid">{d.team.map((member,i)=><article className="team-profile" key={i}><div className="team-profile-head">{member.photoId?<img src={imageUrl(member.photoId,480)} alt={`${member.name}, team member`} loading="lazy"/>:<span className="team-avatar-placeholder" aria-hidden="true">{member.name.slice(0,1)}</span>}<h3>{member.name}</h3></div><dl><div><dt>Role</dt><dd>{member.contribution||'Not listed'}</dd></div><div><dt>College</dt><dd>{member.college||'Not listed'}</dd></div><div><dt>Branch</dt><dd>{member.branch||'Not listed'}</dd></div><div><dt>Semester</dt><dd>{member.semester||'Not listed'}</dd></div></dl></article>)}</div></section>
    <section className="detail-section"><h2>Storage &amp; Deployment</h2><p>The technologies used to store project data and run the application.</p>{d.services.length?<div className="service-grid">{d.services.map((service,i)=><article className="service-card" key={i}><h3>{service.name}</h3><p>{service.purpose||'Supporting service'}</p>{service.url&&<a href={service.url} className="inline-link" target="_blank" rel="noopener noreferrer">Visit service <ArrowUpRight size={14}/></a>}</article>)}</div>:<p>No external services listed.</p>}</section>
    ...
    <div className="panel">
     <div className="sidebar-group"><h2>Build on this idea</h2><div className="sidebar-actions">
