...
export function buildWorkshop(realistic=false){
 ...
 const blue=mat(realistic?'#42464b':'#3159b1',realistic?.85:.3,.3),navy=mat(realistic?'#111518':'#152746',.15,.4),silver=mat(realistic?'#969ca2':'#c6ced8',.85,.3),paper=mat('#efe8d9'),orange=mat(realistic?'#25282b':'#f17d4b',.15,.5),gold=mat('#bfa46c',.9,.22),pcb=mat(realistic?'#143825':'#193f48',.15,.5),black=mat('#17191c',.05,.55),white=mat('#f5f5ee');
 ...
 for(let row=0;row<4;row++)for(let col=0;col<12;col++){
  const x=-1.42+col*.258,z=-.68+row*.263;box(laptop,.218,.035,.182,black,x,.215,z,.018);
 }
 if(realistic){
  // One transparent atlas carries the legends across the actual sculpted keycaps.
  const canvas=document.createElement('canvas');canvas.width=1024;canvas.height=512;const context=canvas.getContext('2d')!;
  context.fillStyle='#e0e0dd';context.font='24px monospace';context.textAlign='center';context.textBaseline='middle';
  for(let row=0;row<4;row++)for(let col=0;col<12;col++)context.fillText(keys[row][col]||'',(col+.5)*1024/12,(row+.5)*512/4);
  const texture=new THREE.CanvasTexture(canvas);texture.colorSpace=THREE.SRGBColorSpace;textures.push(texture);
  const material=new THREE.MeshBasicMaterial({map:texture,transparent:true,depthWrite:false});materials.push(material);
  const geometry=new THREE.PlaneGeometry(3.096,1.052);geometries.set('key-legends',geometry);const legends=mesh(geometry,material,laptop,0,.236,-.285);legends.rotation.x=-Math.PI/2;
  for(const side of [-1,1]){box(laptop,.012,.045,.25,black,side*1.779,.1,-.3,.006);box(laptop,.014,.035,.16,black,side*1.779,.1,.12,.006);}
 }
 ...
 box(lid,3.55,2.26,realistic?.075:.14,blue,0,1.13,0,.04);box(lid,3.35,2.08,.03,navy,0,1.14,.052,.03);
 ...
 const camera=new THREE.Group();group.add(camera);camera.position.set(2.0,.34,-1.15);camera.rotation.y=-.2;if(realistic)camera.visible=false;
