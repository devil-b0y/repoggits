...
import { deliverMail, deliveryMode, openOutboxRow } from '../lib/mail';
async function main(){
  ...
  const rows=(await db.query("SELECT * FROM r.outbox WHERE status='pending' ORDER BY created_at LIMIT 100")).map(openOutboxRow);
  let sent=0;
...
main().catch(()=>{console.error('Email delivery failed. Check the MAIL_MODE settings and DATA_ENCRYPTION_KEY; unsent messages remain queued.');process.exitCode=1;});
