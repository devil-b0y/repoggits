...
The homepage presents an interactive capability diagram, a six-step workflow, a software/hardware/hybrid explorer, and a visual explanation of the project story. It does not show fictional projects, sample creators, or demo project cards. Real projects remain on `/projects`, with existing workspace and submission entry links preserved. Framer Motion animates semantic text blocks throughout the page; animation stops for the pause control and reduced-motion preference without changing accessible text.

Dark mode is the default for new homepage visitors. The light/dark control uses the existing `repoggits-theme` browser preference. Reduced motion, a pause control, keyboard navigation, and dedicated mobile layouts keep the page usable. Framer Motion handles reveals, scroll composition, pointer depth, and feature transitions. The engineer is an original generated still, not a rigged animated character; the surrounding product interfaces are animated independently. Images are delivered through Next.js image optimization. The discipline explorer uses original Three.js laptop and ESP32 geometry with metal finishes, key legends, modeled pin headers, studio HDR lighting, pointer response, and a photographic fallback when WebGL is unavailable.

![Interactive Project Vault capability diagram](docs/screenshots/product-dashboard.png)

![The story behind each engineering project](docs/screenshots/product-case-study.png)
